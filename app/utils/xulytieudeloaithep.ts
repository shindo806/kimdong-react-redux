...
function xuLyTieuDeLoaiThep(loaihang: string, thongso: ThongSoObject) {
  ...
  const m = thongso.day === '' ? 0 : thongso.day;
  const a = thongso.a === '' ? 0 : thongso.a;
  const b = thongso.b === '' ? 0 : thongso.b;
  const c = thongso.c === '' ? 0 : thongso.c;
  const a1 = thongso.a1 === '' ? 0 : thongso.a1;

  const renderEl = document.getElementById('thong-so-title');
